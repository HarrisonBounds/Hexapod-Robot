...
double convertRange(double old_val)
{
  double old_range = DEGREE_MAX - DEGREE_MIN;
  double new_range = MINIMUM_POSITION_LIMIT - MAXIMUM_POSITION_LIMIT; 
  double new_value;

  new_value = (((old_val - DEGREE_MIN) * new_range) / old_range) + MINIMUM_POSITION_LIMIT;

  return new_value;
}

double IK(int x, int y, int z)
{
...
int main() {
  ...
  while(true)
  {
    ...
    printf("Present Position: %03d\n", dxl_present_position);

    degrees = IK(100, 0, 0);
    dxl_rotation = convertRange(degrees);

    if (index == 0)
    {
      new_pos = dxl_present_position + dxl_rotation;
    }
    else if (index == 1)
    {
      new_pos = dxl_present_position - dxl_rotation;
    ...
    dxl_comm_result = packetHandler->write4ByteTxRx(portHandler, DXL_ID, ADDR_GOAL_POSITION, new_pos, &dxl_error);
